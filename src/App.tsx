  ...
  return (
    <div className="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-4">
      <div className="relative bg-slate-800 rounded-2xl p-6 w-full max-w-md border border-slate-700 text-white max-h-[80vh] overflow-y-auto">
        <button onClick={onClose} className="absolute top-3 right-3 p-1 text-slate-400 hover:text-white"><X size={20} /></button>
        <h2 className="text-xl font-bold text-center mb-2">대련상태찾기</h2>
        <p className="text-sm text-slate-300 mb-4 text-center">AI 분석을 통해 승률이 높은 대련 상대자 목록입니다.</p>
        <div className="space-y-4">
          {visible.map(c => (
            <Card key={c.id} className="bg-slate-700/50">
              <div className="flex justify-between items-start">
                <div>
                  <p className="font-bold">{c.name}</p>
                  <p className="text-sm text-slate-300">{c.location} / {c.officialRank} / {c.dojang}</p>
                  <p className="text-sm text-slate-300">검력 {c.experience}년</p>
                  <div className="mt-1">
                    <p className="text-xs text-yellow-300 font-semibold">특징</p>
                    {c.features.map((f, idx) => <p key={idx} className="text-xs text-slate-300">- {f}</p>)}
                  </div>
                  <div className="mt-2">
                    <p className="text-xs text-green-300 font-semibold">승리전략</p>
                    {c.strategy.map((s, idx) => <p key={idx} className="text-xs text-slate-300">- {s}</p>)}
                  </div>
                </div>
...
const HomeScreen = ({ user, onNavigate, notifications, onSelectNotification }) => {
  ...
  <>
    ...
    {modal === 'sparring' && <SparringStatusModal candidates={sparringCandidates} onClose={closeModal} />}

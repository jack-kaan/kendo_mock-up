...
import { Home, Swords, Trophy, User as UserIcon, MapPin, Star, Shield, BarChart2, Settings, Users, MessageSquare, ThumbsUp, Share2, Award, BookOpen, Video, ShieldCheck, CheckCircle2, Store, Tag, X, ThumbsDown, Lock, Unlock, CheckSquare, Square, ArrowUp, ArrowDown, Flame, ChevronDown, Megaphone, Send, FileUp, Plane, Info, Gift, PlusCircle } from 'lucide-react';
...
       const ChatWindow = ({ chat, onBack }) => (
        <div className="h-full flex flex-col">
            <div className="flex items-center p-3 border-b border-slate-700">
                {/* A text-white class has been added to make the button visible */}
...
            <div className="bg-slate-800 rounded-2xl w-full max-w-sm border border-slate-700 text-white relative p-4 flex flex-col max-h-[90vh]">

                <button onClick={onClose} className="absolute top-3 right-3 p-1 text-slate-400 hover:text-white z-20 bg-slate-800/50 rounded-full">
...
                        <div onClick={() => setIsAvatarSpaceOpen(true)} className="w-1/3 flex items-center justify-center cursor-pointer">
...
                        <h3 className="font-semibold text-lg flex items-center"><BarChart2 className="w-5 h-5 mr-2 text-blue-400"/>전적 통계</h3>

                        <button onClick={() => setShowDetailedStats(!showDetailedStats)} className="flex items-center gap-1 text-sm bg-slate-700 px-3 py-1 rounded-lg">

                            상세 <ChevronDown className={cn("transition-transform", showDetailedStats && "rotate-180")} size={16}/>
...
                        <div><p className="text-2xl font-bold">{user.wins + user.losses}</p><p className="text-sm text-slate-400">총 대련</p></div>

                        <div><p className="text-2xl font-bold text-green-400">{user.wins}</p><p className="text-sm text-slate-400">승리</p></div>

                        <div><p className="text-2xl font-bold text-red-400">{user.losses}</p><p className="text-sm text-slate-400">패배</p></div>
...
                {showDetailedStats && <DetailedStatsCard stats={user.detailedStats} />}
...
    const [activeTab, setActiveTab] = React.useState('kka');
